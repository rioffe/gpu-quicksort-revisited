...
import GpuQuicksortProof.GpuQuicksort.Theorems.Progress
import GpuQuicksortProof.GpuQuicksort.Theorems.Gen
...
- *Host orchestration* (`Sorter.lean`): the block layout; the phase-one loop keeps the spec
  ...
  the same result for every parameter choice and every atomic order; the bookkeeping buffers are
  never overrun (K-09).
- *Progress* (`Progress.lean`): live sequences stay disjoint and every host pivot makes both
  children strictly shorter (I-005); all-equal input takes one phase-one iteration and no phase
  two (K-10, E-04).
- *Numbers*: the code's `optp`, in `Float`, reproduces the spec's worked examples (K-05, by
  evaluation); the transcribed MT19937 matches the C++ standard's reference value and the
  generators compute the C-08 formulas (C-08, R-20).
...
- the report fields, diagnostics, timing, the API and CLI surfaces, tuning, packaging, the
  platform, and the `float32` conversion of generated values.
...
`bv_decide` (codec) and `native_decide` (`optp`'s examples, MT19937's reference value) additionally
trust Lean's compiled evaluator.
...
| C-08 (process side) | the `float32` conversion `Float(v).bitPattern` | T-25, T-26 |
| R-20 (process side) | the generated bytes on disk | T-25, T-26 |
| K-09 (process side) | the Metal allocations and the cached auxiliary buffer | T-21 |
