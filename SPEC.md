...
| **R-11** | Phase-one pivots MUST be chosen by the configured `PhaseOnePivot` strategy (C-03). The default is the median of $s_b$, $s_{\lfloor (b+e)/2 \rfloor}$ and $s_{e-1}$ of the sequence $[b, e)$ in its current buffer (`medianOfThree`, [P Alg 1–2]). `minMaxAverage` ([P §5.2]) MAY be selected, and is then specified by O-2. |
...
`algorithm` is one of `gpu-quicksort`, `cpu-swift`, `cpu-qsort`, `cpu-stdsort`. For the `cpu-*` rows, `gpu_ms`, the tuning columns and `tuning_entry` are empty. `os_version` is `ProcessInfo.operatingSystemVersionString`. `--format json` emits an array of objects with the same keys. After the rows, and only with `--verbose`, a summary per (dist, n, algorithm) goes to stderr: the median and min of `wall_ms` and the throughput $n / (\mathrm{median}\ \mathit{wall\_ms} \cdot 10^{3})$ in Mkeys/s (K-11).
